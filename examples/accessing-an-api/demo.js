        ...
        const request = httpRequest.request(url, opts, res => {
            ...
            res.on('end', () => {
                ...
                if(res.statusCode == 200)
                    ...
                    resolve(makeGETRequest(url));

                else
                    reject({ res, error: json.error || '(unknown)' });
            });
        });
...
const getAirports = async () => (await makeGETRequest(buildAPIURI('info/airports'))).json.airports;
...
const echoFlights = flights => flights.forEach(flight => {
    const { flightNumber: num, comingFrom: from, landingAt: to, departingTo: depTo, type, status, gate } = flight;
    const dTo = type == 'departure' ? ` departing to ${depTo}` : '';
    console.log(`Flight ${num} from ${from} to ${to} (${type}, ${status}) is at gate ${gate}${dTo}`);
});
...
(async () => {
    try {
        ...
        const flights2 = await searchFlights({ status: 'landed|arrived|boarding' }, flights1.slice(-1).flight_id);

        console.info('\n10th airport:');
        console.log(`${tenthAp.name} (${tenthAp.shortName}) @ ${tenthAp.city}, ${tenthAp.state}`);
